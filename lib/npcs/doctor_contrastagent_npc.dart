...
const String kContrastAgentBadge = 'ContrastAgentBadge'; // Define the badge ID

class DoctorContrastAgentNPC extends SimpleEnemy with BlockMovementCollision {
  ...
  final List<String> allAgents = ['Gadolinium', 'Kontrastmittel auf Basis von Bariumsulfat', 'Primovist', 'Jodhaltiges Kontrastmittel'];

  DoctorContrastAgentNPC({
        ...
        children: <Widget>[
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Gut, dass wir uns über den Weg laufen. Ich benötige dringend ein spezielles Kontrastmittel für eine MRT-Prostatauntersuchung. Es müsste im Vorratslager des Krankenhauses sein. Können Sie es mir bringen? Achten Sie auf die richtige Kennzeichnung!',
  ...
  void _showSecondDialogue(BuildContext context, PlayerSerife player) {
    ...
    List<String> availableAgents = allAgents.where((agent) => inventory.contains(agent)).toList();

    if (availableAgents.isEmpty) {
      _showNoAgentsDialogue(context, player);
      return;
    }

    _showAgentsSelectionDialogue(context, player, availableAgents);
  }

  void _showNoAgentsDialogue(BuildContext context, PlayerSerife player) {
    _showCustomDialog(
      context: context,
      player: player,
      child: Column(
        ...
        children: <Widget>[
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Ohne das Kontrastmittel können wir die Untersuchung nicht durchführen. Bitte gehen Sie und besorgen Sie es so schnell wie möglich.',
                  style: TextStyle(color: Colors.white, fontSize: 24),
                  textAlign: TextAlign.center,
                ),
              ),
  ...
  void _showAgentsSelectionDialogue(BuildContext context, PlayerSerife player, List<String> availableAgents) {
    _showCustomDialog(
      context: context,
      player: player,
      child: Column(
            ...
            children: [
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Ah, Sie sind zurück. Ich sehe, Sie haben Kontrastmittel gefunden. Welches davon möchten Sie mir für die MRT-Prostatauntersuchung geben?',
                      textAlign: TextAlign.center,
              ...
              const SizedBox(height: 20),
              ..._buildAnswerButtons(context, player, availableAgents),
            ],
          ),
  ...
  List<Widget> _buildAnswerButtons(BuildContext context, PlayerSerife player, List<String> availableAgents) {
    List<Widget> buttons = [];
    for (String agent in availableAgents) {
      buttons.add(
        Padding(
          padding: const EdgeInsets.only(bottom: 8.0),
          child: TextButton(
            child: Text(agent, style: TextStyle(color: Colors.white, fontSize: 20)),
            onPressed: () {
              Navigator.of(context).pop();
              _checkAnswer(context, player, agent);
            },
          ),
        ),
      );
  ...
  void _checkAnswer(BuildContext context, PlayerSerife player, String agent) {
    if (agent == 'Gadolinium') {
      player.removeCollectible(agent);
        ...
        children: <Widget>[
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Moment mal... Das ist nicht das richtige Kontrastmittel. Bitte bringen Sie mir das richtige Kontrastmittel für eine MRT-Untersuchung.',
                  style: TextStyle(color: Colors.white, fontSize: 24),
                  textAlign: TextAlign.center,
                ),
              ),
...
// Usage example
DoctorContrastAgentNPC createDoctorContrastAgentNPC(Vector2 position) {
  return DoctorContrastAgentNPC(
    position: position,
    ...
    imagePath: 'assets/images/characters/doctor_xray.png', // Provide the image path here
    badgeImagePath: 'assets/images/objects/BadgeContrastAgent-large.png', // Provide the image path here
    badgeID: kContrastAgentBadge,
  );
}
